...
<div class="flex flex-row">
    ...
    on:mouseenter="{() => current = `${mod.Title}`}" style="background-image: linear-gradient(1deg, rgb(0 0 0 / 0%), rgb(0 0 0 / 90%)), url('{mod.image}')">
        <div class="object-center mx-[10%] mt-28 flex flex-col justify-center items-center">
            ...
            <a class="mt-16 inline-flex py-2 px-7 text-l font-medium text-center rounded-full shadow-xl text-white bg-sky-500 border hover:text-sky-500 hover:font-bold hover:border border-sky-500 hover:bg-white hover:bg-opacity-60" href="/dashboard/${mod.Title}${$dets.newUrl}" data-sveltekit-prefetch type="button" 
