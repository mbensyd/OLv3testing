<script>
    ...
    // one way to get xAPI data from url
...
<div class="bg-cover bg-center w-full h-screen" style="background-image: url('{bgspace}')">
    ...
    <div class="mt-32 flex justify-center items-center" >
        <div class="p-16 max-w-2xl bg-white bg-opacity-70 rounded-3xl shadow-2xl dark:bg-gray-800 dark:border-gray-700">
            ...
            <a data-sveltekit-prefetch href={`/dashboard${newUrl}`} class="mt-2 inline-flex py-2 px-7 text-l font-medium text-center rounded-full shadow-xl text-white bg-sky-500 border hover:text-sky-500 hover:font-bold hover:border border-sky-500 hover:bg-white hover:bg-opacity-60" >
                LAUNCH LEARN SPACE
            </a>
        </div>
    </div>
