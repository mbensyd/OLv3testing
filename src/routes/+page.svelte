...
<div class="bg-cover bg-center w-full h-screen" style="background-image: url('{bgspace}')">
    ...
    <div class="mt-32 flex justify-center items-center">
        <div class="p-16 max-w-2xl bg-white bg-opacity-70 rounded-3xl shadow-2xl dark:bg-gray-800 dark:border-gray-700">
            ...
            <a data-sveltekit-prefetch href={`/transactions/?activity_id=https%3A%2F%2Fwww.openlearning.com%2Fcourses%2Fanother-experiment%2Fhomepage%2F%3Fblock%3D633578b241a8884a8cb4a08b%26embedded%3Dtrue&actor=%7B%22objectType%22%3A%22Agent%22%2C%22name%22%3A%22${nameURL}%22%2C%22mbox%22%3A%22${mboxURL}%22%7D&auth=${authURL}&endpoint=${endpointURL}&registration=${regURL}`} class="mt-2 inline-flex py-2 px-7 text-l font-medium text-center rounded-full shadow-xl text-white bg-sky-500 border hover:text-sky-500 hover:font-bold hover:border border-sky-500 hover:bg-white hover:bg-opacity-60" >
                LAUNCH LEARN SPACE
            </a>
        </div>
    </div>
